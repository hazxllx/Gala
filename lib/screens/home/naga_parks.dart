...
import 'naga_parks/ecopark.dart';

class ParksPage extends StatefulWidget {
  final String locationName;
  const ParksPage({super.key, required this.locationName});
...
class _ParksPageState extends State<ParksPage> {
  ...
  List<ParkData> allParks = [
    ParkData(
      ...
      title: 'Naga Ecology Park',
      subtitle: 'Zone 6, San Felipe, Naga City',
      page: const EcoparkPage(),
    ),
  ];

  List<ParkData> nearbyParks = [];

  bool get isNaga => widget.locationName.toLowerCase().contains('naga');
  bool get isSearching => _searchController.text.isNotEmpty;

  List<ParkData> get _filteredParks {
    if (!isNaga) return [];
    final query = _searchController.text.toLowerCase();
    if (query.isEmpty) return [];
    final allParksList = [...allParks, ...nearbyParks];
    return allParksList.where((park) {
      return park.title.toLowerCase().contains(query) ||
          park.subtitle.toLowerCase().contains(query);
    }).toList();
  }

  User? user;
  String? userPhotoUrl;
  Stream<Set<String>> favoriteParkTitlesStream = Stream.value(<String>{});
  ...
  void initState() {
    ...
    if (user != null) {
      favoriteParkTitlesStream = FirebaseFirestore.instance
          .collection('users')
          .doc(user!.uid)
  ...
  void _sortParks(String sortType, List<ParkData> parksToSort) {
    setState(() {
      if (sortType == 'A-Z') {
        parksToSort.sort((a, b) => a.title.toLowerCase().compareTo(b.title.toLowerCase()));
      } else if (sortType == 'Z-A') {
        parksToSort.sort((a, b) => b.title.toLowerCase().compareTo(a.title.toLowerCase()));
      }
    });
  }

  void _sortAllParks(String sortType) {
    _sortParks(sortType, allParks);
  }

  Future<void> _toggleFavorite(ParkData park, Set<String> favoriteParkTitles) async {
    if (user == null) {
      if (mounted) {
    ...
    final isFav = favoriteParkTitles.contains(park.title);
  ...
  Widget _buildParkCard(ParkData park, Set<String> favoriteParkTitles) {
    final isFavorite = favoriteParkTitles.contains(park.title);
    ...
    return GestureDetector(
      onTap: () {
        if (park.page != null) {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => park.page!),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('No details page yet for ${park.title}')),
          );
        }
      },
      child: ParkCard(
        ...
        onFavoriteToggle: () => _toggleFavorite(park, favoriteParkTitles),
        user: user,
      ),
    );
  }
                ...
                children: [
                  ...
                  if (isSearching && isNaga) ...[
                    StreamBuilder<Set<String>>(
                      stream: favoriteParkTitlesStream,
                      builder: (context, snapshot) {
                        final favoriteParkTitles = snapshot.data ?? <String>{};
                          ...
                          return SizedBox(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return _buildParkCard(park, favoriteParkTitles);
                              },
                            ),
                          );
                        }
                      ...
                      builder: (context, snapshot) {
                        ...
                        return SizedBox(
                          ...
                          child: ListView(
                            scrollDirection: Axis.horizontal,
                            padding: const EdgeInsets.symmetric(horizontal: 4),
                            children: allParks.map((park) => _buildParkCard(park, favoriteParkTitles)).toList(),
                          ),
                        );
                      },
                    ),
