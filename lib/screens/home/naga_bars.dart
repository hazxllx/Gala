...
class BarsPage extends StatefulWidget {
  ...
  const BarsPage({super.key, required this.locationName});
...
class _BarsPageState extends State<BarsPage> {
  ...
  List<BarData> allBars = [
    BarData(
      imagePath: 'https://gala-app-images.s3.ap-southeast-2.amazonaws.com/naga_bars/courtside/courtside.jpg',
      title: 'Courtside',
      subtitle: 'Dinaga St., Naga City',
    ),
    BarData(
      imagePath: 'https://gala-app-images.s3.ap-southeast-2.amazonaws.com/naga_bars/barcode/barcode.jpg',
      title: 'BarCode Sports Bar DYD',
      subtitle: 'Dayandang, Naga City',
    ),
    BarData(
      imagePath: 'https://gala-app-images.s3.ap-southeast-2.amazonaws.com/naga_bars/bora/bora.jpg',
      title: 'Bora Hut',
      subtitle: 'Peñafrancia Ave., Naga City',
    ),
  ];

  List<BarData> nearbybars = [];

  bool get isNaga => widget.locationName.toLowerCase().contains('naga');
  bool get isSearching => _searchController.text.isNotEmpty;

  List<BarData> get _filteredBars {
    if (!isNaga) return [];
    final query = _searchController.text.toLowerCase();
    if (query.isEmpty) return [];
    final allBarsList = [...allBars, ...nearbybars];
    return allBarsList.where((bar) {
      return bar.title.toLowerCase().contains(query) ||
          bar.subtitle.toLowerCase().contains(query);
    }).toList();
  }

  User? user;
  String? userPhotoUrl;
  Stream<Set<String>> favoriteBarTitlesStream = Stream.value(<String>{});
  ...
  void initState() {
    ...
    if (user != null) {
      favoriteBarTitlesStream = FirebaseFirestore.instance
          .collection('users')
          .doc(user!.uid)
  ...
  void _sortBars(String sortType, List<BarData> barsToSort) {
    setState(() {
      if (sortType == 'A-Z') {
        barsToSort.sort((a, b) => a.title.toLowerCase().compareTo(b.title.toLowerCase()));
      } else if (sortType == 'Z-A') {
        barsToSort.sort((a, b) => b.title.toLowerCase().compareTo(a.title.toLowerCase()));
      }
    });
  }

  void _sortAllBars(String sortType) {
    _sortBars(sortType, allBars);
  }
  ...
  Future<void> _toggleFavorite(BarData bar, Set<String> favoriteBarTitles) async {
    ...
    final isFav = favoriteBarTitles.contains(bar.title);
  ...
  Widget _buildBarCard(BarData bar, Set<String> favoriteBarTitles) {
    final isFavorite = favoriteBarTitles.contains(bar.title);

    return GestureDetector(
      onTap: () {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('No details page yet for ${bar.title}')),
        );
      },
      child: BarCard(
        imagePath: bar.imagePath,
        ...
        onFavoriteToggle: () => _toggleFavorite(bar, favoriteBarTitles),
        user: user,
      ),
    );
  }
                ...
                children: [
                      ...
                      text: TextSpan(
                        ...
                        children: [
                          const TextSpan(
                            text: 'Bars',
                            style: TextStyle(
                              color: Color.fromARGB(255, 31, 166, 35),
                              fontFamily: 'Inter',
                              fontSize: 32,
                  ...
                  if (isSearching && isNaga) ...[
                    StreamBuilder<Set<String>>(
                      stream: favoriteBarTitlesStream,
                      builder: (context, snapshot) {
                        final favoriteBarTitles = snapshot.data ?? <String>{};
                        if (_filteredBars.isEmpty) {
                          return Center(
                            child: Padding(
                          ...
                          return SizedBox(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return _buildBarCard(bar, favoriteBarTitles);
                              },
                            ),
                          );
                        }
                      ...
                      builder: (context, snapshot) {
                        ...
                        return SizedBox(
                          ...
                          child: ListView(
                            scrollDirection: Axis.horizontal,
                            padding: const EdgeInsets.symmetric(horizontal: 4),
                            children: allBars.map((bar) => _buildBarCard(bar, favoriteBarTitles)).toList(),
                          ),
                        );
                      },
                    ),
